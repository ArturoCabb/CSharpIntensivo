...
{
    internal static class Introduccion
    {
        public static async Task otroMasAsync()
        {
            var httpClient = new HttpClient();

            var respuesta = await httpClient.GetAsync("http://localhost:5186/WeatherForecast");
            ...
            Console.WriteLine("-- codigo de estatus --");
            Console.WriteLine(respuesta.StatusCode);
            Console.WriteLine(respuesta.IsSuccessStatusCode);

            // uso GetStringAsync
            var respuestaString = await httpClient.GetStringAsync("http://localhost:5186/WeatherForecast");
            Console.WriteLine(respuestaString);

            // Accion a status code
